...
def fill_coords(
    img: ndarray[np.uint8], fn: FilterFunction, color: ndarray[np.uint8]
) -> ndarray[np.uint8]:
    """
...
def rotate_fn(
    fin: FilterFunction, cx: float, cy: float, theta: float
) -> FilterFunction:
    """
...
def highlight_img(
    img: ndarray[np.uint8], color: ndarray[np.uint8] = White, alpha=0.30
) -> ndarray[np.uint8]:
    """
